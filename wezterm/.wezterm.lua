...
for _, direction in ipairs({ "Left", "Right", "Up", "Down" }) do
	...
	table.insert(
		config.keys,
		{ key = direction .. "Arrow", mods = "CMD|OPT", action = act.ActivatePaneDirection(direction) }
	)

	-- resize panes
	table.insert(
		config.keys,
		{ key = direction .. "Arrow", mods = "CTRL|CMD", action = act.AdjustPaneSize({ direction, 3 }) }
	)
	...
	if direction == "Left" or direction == "Right" then
		...
		table.insert(
			config.keys,
			{ key = direction .. "Arrow", mods = "CMD", action = act.ActivateTabRelative(relative) }
		)
		...
		table.insert(
			config.keys,
			{ key = direction .. "Arrow", mods = "CMD|SHIFT", action = act.MoveTabRelative(relative) }
		)
	else
		-- scroll up using option+arrow
		table.insert(
			config.keys,
			{ key = direction .. "Arrow", mods = "OPT", action = act.ScrollByPage(direction == "Up" and -0.2 or 0.2) }
		)

		-- scroll to last command
		table.insert(
			config.keys,
			{ key = direction .. "Arrow", mods = "CMD", action = act.ScrollToPrompt(direction == "Up" and -1 or 1) }
		)
	end
end
